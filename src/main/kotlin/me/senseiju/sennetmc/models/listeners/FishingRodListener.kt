...
class FishingRodListener(plugin: SennetMC) : Listener {
    ...
    private fun onPlayerFishEvent(e: PlayerFishEvent) {
        if (!ModelType.isItemModel(e.player.inventory.itemInMainHand)) {
            return
        }
    ...
    @EventHandler
    private fun onPlayerDropFishingRod(e: EntityDropItemEvent) {
        if (e.entityType == EntityType.PLAYER && e.itemDrop.itemStack.type == Material.FISHING_ROD) {
            e.isCancelled = true
        }
    }
}